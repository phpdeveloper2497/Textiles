<?php

namespace App\Policies;

use App\Models\BoxHistory;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class BoxHistoryPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->hasRole(['admin','warehouse_manager']);
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole(['admin','warehouse_manager']);
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->hasRole(['admin','warehouse_manager']);
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole('admin',);
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole(['admin','warehouse_manager']);
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole('admin');
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole('admin');
    }

    public function workshop(User $user, BoxHistory $boxHistory): bool
    {
        return $user->hasRole(['admin','warehouse_manager']);
    }
}
